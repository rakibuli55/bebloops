...
const CreatePost = () => {
  ...
  const {register, control, handleSubmit, setValue, getValues, formState: {errors}, trigger} = useForm({
    defaultValues: {
      videoFiles: [],
      pdfFiles: [],
      titleName:'',
  ...
  return (
    <div>
      ...
      <DialogContent className="max-w-[682px] max-h-[720px] overflow-y-auto rounded-[12px] bg-[#F8F8F8]">
        <div>
          <h3 className="text-[24px] font-semibold text-themeBlack text-center mb-[48px]">Create a Movement</h3>
          <form onSubmit={handleSubmit(onSubmit)}>
            <Controller 
             ...
             <Controller
                ...
                render={({ field: { value } }) => (
                  <>
                    <div
                      {...getPDFRootProps()}
                      className={`border-[2px] min-h-[160px] border-dashed border-[#B5B5B5] rounded-[10px] flex items-center justify-center bg-white py-10 px-[55px] ${isDragActiveVideo ? 'bg-[rgba(230,230,255,0.43)]' : ''} mt-4 ${errors.pdfFiles ? 'border-red-500' : 'border-[#B5B5B5]'}`}
                    >
                      <input {...getPDFInputProps()} />
                      {
                        isDragActivePDF ? <p>Drop the files here ...</p> : <div className="flex items-center flex-col justify-center">
                        <p className="text-[40px] text-sidebarIconColor">
                            <BiSolidFilePdf />
                        </p>
                        {
                        value && value.length > 0 ? (
                          ...
                          ) : (<p className="text-base font-medium mt-2">Drag & drop and your pdf or <span className="text-primaryColor cursor-pointer hover:opacity-50 duration-100 ease-in-out">browse here..</span></p>)
                        }
                      </div>
                      }
                    </div>
             ...
             <button type="submit" className="block w-full text-sm text-white font-medium bg-primaryColor text-clip p-3 rounded-[6px] mt-[30px] duration-200 ease-in-out hover:bg-[#4b51ef]">Publish Your Movement</button>
          </form>
        </div>
      </DialogContent>
    </Dialog>
